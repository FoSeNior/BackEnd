...
public class UserController {
    ...
    public ResponseEntity<Map<String, Object>> isUserIdAvailable(@RequestBody Map<String, String> request) {
        String membershipId = request.get("userId"); // JSON에서 userId 추출
        boolean available = userService.isUserIdAvailable(membershipId);
        ...
        if (available) {
            ...
            userInfo.put("membershipId", membershipId);
    ...
    // 사용자 정보 조회
    @GetMapping("/{u_id}")
    public ResponseEntity<Map<String, Object>> getUserById(@PathVariable("u_id") int userId) {
